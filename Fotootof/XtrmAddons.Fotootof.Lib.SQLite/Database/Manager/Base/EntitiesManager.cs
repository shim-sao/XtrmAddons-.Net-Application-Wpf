    ...
    public abstract class EntitiesManager
    {
       ...
       /// <returns></returns>
        ...
        public async Task<int> DeleteDependencyAsync(EntityManagerDeleteDependency definition, int entityPK, IEnumerable<int> depdencyPKs, string search = "IN")
        {
            ...
            if (depdencyPKs == null || depdencyPKs.Count() == 0)
            {
                log.Debug($"{GetType().Name}.{MethodBase.GetCurrentMethod().Name} : Affected Rows => 0");
                return 0;
            }
            ...
            string pks = string.Join(",", depdencyPKs);
            log.Debug(query = $"DELETE FROM {QN(definition.Name)} WHERE {QN(definition.key)} = {Q(entityPK.ToString())} AND {QN(definition.keyList)} {search} ({pks})");
            try
            {
